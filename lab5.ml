...
let validated_date ({year; month; day} as date) : date =
  ...
  else let leap = (year mod 4 = 0 && year mod 100 <> 0)|| year mod 400 = 0 in
  let max_days =
  match month with
  | 1 
  | 3 
...
let new_child (name : string) (col : color) (birth : date) : family = 
  Single { name; favorite = validated_rgb col; birthdate = validated_date birth } ;;
...
let marry (fam : family) (spouse : person) : family =
  ...
  | Family _ -> raise (Family_Trouble ("cannot add "^ spouse.name ^ " to a couple")) ;;
